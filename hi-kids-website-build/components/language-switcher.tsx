...
export function LanguageSwitcher({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="relative group">
      <button
        className="flex items-center gap-2 rounded-2xl px-4 py-2.5 text-sm font-black uppercase tracking-widest text-white/90 hover:text-white hover:bg-white/10 border border-transparent hover:border-white/20 transition-all"
      ...
      <div className="absolute right-0 top-full pt-1 opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-300 z-50">
        <div className="min-w-[180px] rounded-3xl border border-border bg-white p-2 shadow-3xl">
          {i18n.locales.map((l) => (
            ...
            >
              <span className="font-black tracking-widest uppercase">{localeLabels[l]}</span>
              <span className={`text-xs font-bold ${l === locale ? "text-white/80" : "text-muted-foreground/60"}`}>{localeNames[l]}</span>
            </button>
          ))}
        </div>
      </div>
