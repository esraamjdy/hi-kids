    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed bottom-8 left-8 z-[100] flex items-end gap-6"
        >
          {/* 1. APP BUTTON (The Phone) */}
          <Link
          ...
          {/* 2. FRANCHISE / INQUIRY BUTTON (The Global Link) */}
          <Link
            href={`/${locale}/franchise/inquiry`}
            className="group relative flex flex-col items-center mb-1"
          >
            {/* Minimalist Square Tablet/Card Frame */}
            <div className="relative w-20 h-20 bg-white rounded-[2rem] border-[3px] border-hikids-blue shadow-[0_15px_40px_rgba(0,174,239,0.15)] group-hover:shadow-hikids-blue/40 transition-all group-hover:-translate-y-3 group-hover:-rotate-3 duration-500 flex items-center justify-center overflow-hidden">
               <div className="absolute inset-0 bg-hikids-blue/5 opacity-0 group-hover:opacity-100 transition-opacity" />
               <Building2 className="text-hikids-blue w-9 h-9 group-hover:scale-110 transition-transform duration-500" />
            </div>

            {/* Franchise Label */}
            <div className="absolute -right-2 top-1/2 -translate-y-1/2 translate-x-full opacity-0 group-hover:opacity-100 group-hover:translate-x-4 transition-all duration-500 pointer-events-none">
              <div className="bg-hikids-blue text-white px-5 py-3 rounded-2xl shadow-2xl flex flex-col items-start whitespace-nowrap border border-white/10">
                <span className="text-[10px] font-black font-fredoka flex items-center gap-2 uppercase tracking-widest">Open a Franchise <ArrowRight size={10} className="animate-pulse" /></span>
              </div>
            </div>
          </Link>
        </motion.div>
