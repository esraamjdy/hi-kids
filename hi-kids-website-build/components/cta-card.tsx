...
export function CtaCard({
  ...
  const variants = {
    ...
    secondary: {
      wrapper: "hover:border-hikids-green/30 hover:shadow-hikids-green/5",
      icon: "bg-hikids-green/10 text-hikids-green",
      cta: "text-hikids-green",
    },
    accent: {
      wrapper: "hover:border-hikids-yellow/50 hover:shadow-hikids-yellow/10 border-hikids-yellow/20 bg-hikids-yellow/5",
  ...
  return (
    ...
    >
      <div className={`flex h-16 w-16 items-center justify-center rounded-2xl transition-colors ${style.icon}`}>
        {icon}
      </div>
      <h3 className="text-xl font-bold text-card-foreground">{title}</h3>
      <p className="text-muted-foreground leading-relaxed font-medium">{description}</p>
      <span className={`mt-auto inline-flex items-center gap-2 font-bold group-hover:gap-3 transition-all ${style.cta}`}>
        {cta}
